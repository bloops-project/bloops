...
func (r *Session) checkChallengeSendMsg(player *Player) (int, error) {
...
func (r *Session) sendDroppedBloopsesMsg(player *Player, bloops *resource.Bloops) error {
...
func (r *Session) sendLetterMsg(player *Player) error {
	buf := strpool.Get()
	defer func() {
		buf.Reset()
		strpool.Put(buf)
...
func (r *Session) sendFreezeTimerMsg(player *Player, secs int) (int, error) {
	var messageId int
	buf := strpool.Get()
	defer func() {
		buf.Reset()
...
func (r *Session) sendWorkingTimerMsg(player *Player, messageId, secs int) error {
	buf := strpool.Get()
	defer func() {
		buf.Reset()
		strpool.Put(buf)
...
func (r *Session) sendChoiceBloopsMsg(ctx context.Context, player *Player) error {
